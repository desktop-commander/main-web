...
const TestimonialsStrip = ({ testimonials }: TestimonialsStripProps) => {
  const TestimonialCard = ({ testimonial, index }: { testimonial: Testimonial; index: number }) => {
    const rotation = index % 4 === 0 ? 'rotate-1' : index % 4 === 1 ? '-rotate-1' : index % 4 === 2 ? 'rotate-2' : '-rotate-2';
    ...
    return (
      <div className={`flex-shrink-0 w-80 p-4 ${rotation} hover:rotate-0 transition-transform duration-300`}>
        <div className="lp-ring-subtle rounded-xl p-5 h-56 flex flex-col shadow-lg shadow-black/30 transition-shadow duration-300 hover:shadow-[0_16px_50px_-16px_hsl(var(--dc-blue)/0.35)]">
          <div className="flex items-center gap-3 mb-3 flex-shrink-0">
            <div className={`w-9 h-9 ${generateAvatarColor(testimonial.author)} rounded-full flex items-center justify-center ring-1 ring-white/10`}>
            ...
            <div>
              ...
              <div className="font-mono text-muted-foreground text-[10px]">{generateRandomDate(testimonial.author)}</div>
            </div>
          </div>
          <div className="text-foreground/90 text-sm leading-relaxed flex-1 overflow-y-auto scrollbar-thin scrollbar-track-gray-700 scrollbar-thumb-gray-500 hover:scrollbar-thumb-gray-400">
            {testimonial.quote}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="w-full overflow-hidden lp-fade-x lp-pause-hover">
